...
export interface VditorEditorConfig {
  ...
  /** Editor mode */
  mode?: 'sv' | 'ir' | 'wysiwyg';
  /** Language for Vditor UI (e.g., 'en_US', 'zh_CN') */
  lang?: string;
...
export class VditorEditor {
  ...
  /**
   * Insert markdown table with specified dimensions
   * @param rows - Number of rows (including header)
  ...
  /**
   * Undo last action
   * Note: Vditor doesn't expose undo/redo in public API, so we access internal vditor.undo
   */
  undo(): void {
    if (!this._instance) {
      throw new Error('VditorEditor: Editor not initialized');
    ...
    this._instance.focus();
    // Access internal undo API: vditor.vditor.undo.undo(vditor.vditor)
    const vditor = (this._instance as unknown as { vditor: { undo: { undo: (v: unknown) => void }; currentMode: string } }).vditor;
    if (vditor?.undo) {
  ...
  redo(): void {
    ...
    this._instance.focus();
    // Access internal redo API: vditor.vditor.undo.redo(vditor.vditor)
    const vditor = (this._instance as unknown as { vditor: { undo: { redo: (v: unknown) => void } } }).vditor;
    if (vditor?.undo) {
  ...
  private _buildOptions(): VditorOptions {
    const { theme = 'light', initialValue = '', placeholder, mode = 'ir', lang, customOptions = {}, onChange, onFocus, onBlur } = this._config;
