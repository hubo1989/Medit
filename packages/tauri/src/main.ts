...
import { PreferencesPanel, FindReplacePanel, PreviewToolbar, EditToolbar, TocService, TabBar, AboutDialog } from './ui/index.js';
import { I18nService, type Language } from './i18n/index.js';
import { MenuService } from './menu/index.js';
import { PreferencesService, ThemeService, SyncScrollService, TabManager } from './services/index.js';
import type { TabState, CreateTabOptions } from './types/tab.js';
...
class MeditApp {
  ...
  private _aboutDialog: AboutDialog | null = null;
  ...
  private _tabBar: TabBar | null = null;
  ...
  async init(): Promise<void> {
    ...
    // Setup preference change listeners for tab settings
    this._setupTabPreferenceListeners();
  ...
  private _initEditToolbar(): void {
    ...
    this._editToolbar = new EditToolbar({
      ...
      onInsertTable: (rows, cols) => {
        this._handleInsertTable(rows, cols);
      },
    });
  ...
  private _handleEditAction(action: string): void {
    ...
    switch (action) {
      ...
      case 'heading3':
        ...
        break;
      case 'heading4':
        this._editor.insertHeading(4);
        break;
      case 'heading5':
        this._editor.insertHeading(5);
        break;
      case 'codeBlock':
   ...
   */
  private _handleInsertTable(rows: number, cols: number): void {
    if (!this._editor) return;
    this._editor.insertTable(rows, cols);
  }
  ...
  private _initEditor(): void {
    this._editor = new VditorEditor({
      ...
      placeholder: this._i18n.t('editor.placeholder') ?? '开始编写 Markdown...',
      mode: 'ir',
      lang: this._i18n.getCurrentLanguage() === 'zh' ? 'zh_CN' : 'en_US',
      onChange: (value) => {
  ...
  /**
   * Initialize tab bar for multi-document editing
   */
  private _initTabBar(): void {
    const tabBarContainer = document.getElementById('tab-bar-container');
    if (!tabBarContainer) {
      logger.warn('Tab bar container not found');
      return;
    }

    // Create tab manager
    const maxTabs = this._preferences.get<number>('tabs.maxTabs') ?? 20;
    this._tabManager = new TabManager({
      maxTabs,
      restoreOnStartup: true,
      defaultNewTabBehavior: 'blank',
      onTabClose: (tabId: string) => {
        void this._handleTabClose(tabId);
      },
      onTabSwitch: (tabId: string, tab: TabState) => {
        this._handleTabSwitch(tab);
      },
    });
    ...
    this._tabBar = new TabBar({
      ...
      onTabClose: async (tabId: string) => {
        return await this._handleTabClose(tabId);
      },
    });
    ...
    if (this._tabManager.getTabCount() === 0) {
      this._tabManager.createTab({
        title: this._i18n.t('tabs.untitled'),
        content: '',
      });
    }

    logger.debug('Tab bar initialized');
  }

  /**
   * Handle new tab creation
   */
  private _handleNewTab(): void {
    console.log('[MeditApp] _handleNewTab called');
    if (!this._editor) {
      console.warn('[MeditApp] Editor not initialized');
      return;
    }
    
    // Get new tab content based on preference
    const defaultBehavior = this._preferences.get('tabs.defaultNewTabBehavior') as string || 'blank';
    const content = defaultBehavior === 'welcome' ? this._getWelcomeContent() : '';
    
    console.log('[MeditApp] Creating new tab with content length:', content.length);
    const tab = this._tabManager?.createTab({
      title: this._i18n.t('tabs.untitled'),
      content,
    });
    
    console.log('[MeditApp] Tab created:', tab?.id);
    if (tab) {
   ...
   * Handle tab close
   */
  private async _handleTabClose(tabId: string): Promise<boolean> {
    const tab = this._tabManager?.getTab(tabId);
    if (!tab) return true;

    // Check if tab has unsaved changes
    if (tab.isDirty) {
      const confirmed = confirm(this._i18n.t('tabs.confirmClose'));
      if (!confirmed) return false;
    }

    // Check if this is the last tab
    const isLastTab = this._tabManager?.getTabCount() === 1;
    
    if (isLastTab) {
      // Confirm exit for last tab
      const confirmed = confirm(this._i18n.t('tabs.confirmExit'));
      if (!confirmed) return false;
    ...
    // If this is the active tab, save its state first
    if (this._tabManager?.getActiveTab()?.id === tabId) {
   ...
   * Handle tab switch
   */
  private _handleTabSwitch(tab: TabState): void {
    ...
    if (this._editor) {
      ...
      this._currentContent = tab.content;
      
      // Restore scroll positions
      if (tab.scrollPosition !== undefined) {
        const previewContainer = document.getElementById('preview-container');
        if (previewContainer) {
          previewContainer.scrollTop = tab.scrollPosition;
        }
      }
      
      if (tab.editorScrollPosition !== undefined) {
        // Restore editor scroll position
        const vditorEditor = document.getElementById('vditor-editor');
        const scrollEl = vditorEditor?.querySelector('.vditor-sv') as HTMLElement | null;
        if (scrollEl) {
          scrollEl.scrollTop = tab.editorScrollPosition;
        }
      }
    }
    
    // Update preview
    this._updatePreview(tab.content);
    
    // Update file path if tab has a file
    if (tab.filePath) {
      this._state.filePath = tab.filePath;
    }
  }
   ...
   */
  private _saveCurrentTabState(): void {
    const activeTab = this._tabManager?.getActiveTab();
    if (!activeTab || !this._editor) return;
    ...
    this._tabManager?.updateTabContent(activeTab.id, content, {
      scrollPosition: previewContainer?.scrollTop,
      editorScrollPosition: scrollEl?.scrollTop,
    });
  }

  /**
   * Get welcome content for new tabs
   */
  private _getWelcomeContent(): string {
    return `# ${this._i18n.t('editor.welcomeTitle')}

${this._i18n.t('editor.welcomeSubtitle')}

## ${this._i18n.t('editor.features')}

- Markdown editing with live preview
- Export to WeChat and Zhihu formats
- Diagram support (Mermaid, Vega-Lite, Graphviz)
- Dark and light themes

## ${this._i18n.t('editor.gettingStarted')}
...
`;
  ...
  /**
   * Setup listeners for tab preference changes
   */
  private _setupTabPreferenceListeners(): void {
    // Max tabs changes
    this._preferences.onChange('tabs.maxTabs', (_key, newValue) => {
      const maxTabs = newValue as number;
      this._tabManager?.setMaxTabs(maxTabs);
      logger.debug(`Max tabs changed to ${maxTabs}`);
    });
  }
  ...
  private async _updateMenuLabels(): Promise<void> {
    try {
      const labels: Record<string, string> = {
        // Main menus
        app: this._i18n.t('menu.app'),
        file: this._i18n.t('menu.file'),
        edit: this._i18n.t('menu.edit'),
        view: this._i18n.t('menu.view'),
        help: this._i18n.t('menu.help'),
        // App menu items (macOS)
        'app:about': this._i18n.t('menu.about'),
        'app:preferences': this._i18n.t('menu.preferences'),
        'app:quit': this._i18n.t('menu.quit'),
        // File menu items
