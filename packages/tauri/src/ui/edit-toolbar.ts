...
import { HeadingMenu } from './heading-menu.js';
...
export class EditToolbar {
  ...
  private _onInsertTable?: (rows: number, cols: number) => void;
  ...
  /** Heading menu instance */
  private _headingMenu: HeadingMenu | null = null;
  /** Table menu instance */
  private _tableMenu: TableMenu | null = null;
  ...
  constructor(config: EditToolbarConfig) {
    ...
    this._onInsertTable = config.onInsertTable;
  ...
  private _render(): void {
    ...
    // Undo/Redo buttons (right after mode switch)
    ...
    this._container.appendChild(undoBtn);
    this._container.appendChild(redoBtn);
  ...
  private _createEditActionGroup(): HTMLElement {
    ...
    // Heading menu (dropdown with H1-H5)
    this._headingMenu = new HeadingMenu({
      onSelect: (level) => this._onEditAction?.(`heading${level}` as EditAction),
      i18n: this._i18n,
    });
    const headingBtn = this._headingMenu.createButton();
    ...
    // Table menu
    this._tableMenu = new TableMenu({
      onSelect: (rows, cols) => this._onInsertTable?.(rows, cols),
      i18n: this._i18n,
    });
    const tableBtn = this._tableMenu.createButton();
  ...
  private _setupEventListeners(): void {
    ...
    // Edit actions (headings handled by HeadingMenu)
  ...
  private _updateLabels(): void {
    ...
    // Update heading menu labels
    this._headingMenu?.updateLabels();
    // Update table menu labels
    this._tableMenu?.updateLabels();
  }
  ...
  destroy(): void {
    ...
    this._headingMenu?.destroy();
    this._tableMenu?.destroy();
    this._buttons.clear();
