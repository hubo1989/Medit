...
export type SettingsCategory = 'general' | 'editor' | 'preview' | 'tabs' | 'shortcuts';
...
const CATEGORY_ICONS: Record<SettingsCategory, string> = {
  ...
  tabs: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9h18v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9Z"/><path d="M3 9V7a2 2 0 0 1 2-2h4"/><path d="M9 5h6"/><path d="M15 5h4a2 2 0 0 1 2 2v2"/></svg>`,
...
export class PreferencesPanel {
  ...
  private _getCategories(): CategoryConfig[] {
    ...
    return [
      ...
      {
        id: 'tabs',
        label: t('tabs.newTab') === 'tabs.newTab' ? 'Tabs' : t('tabs.newTab').replace(' New Tab', 's'),
        icon: CATEGORY_ICONS.tabs,
        fields: [
          {
            key: 'tabs.restoreOnStartup',
            type: 'checkbox',
            label: t('tabs.restoreOnStartup'),
            description: t('preferences.desc.restoreOnStartup') === 'preferences.desc.restoreOnStartup'
              ? 'Restore previously open tabs when starting the application'
              : t('preferences.desc.restoreOnStartup'),
          },
          {
            key: 'tabs.maxTabs',
            type: 'number',
            label: t('tabs.maxTabs'),
            description: t('preferences.desc.maxTabs') === 'preferences.desc.maxTabs'
              ? 'Maximum number of tabs that can be open simultaneously'
              : t('preferences.desc.maxTabs'),
            min: 1,
            max: 50,
            step: 1,
          },
          {
            key: 'tabs.defaultNewTabBehavior',
            type: 'select',
            label: t('tabs.defaultNewTabBehavior'),
            description: t('preferences.desc.defaultNewTabBehavior') === 'preferences.desc.defaultNewTabBehavior'
              ? 'Default content for newly created tabs'
              : t('preferences.desc.defaultNewTabBehavior'),
            options: [
              { value: 'blank', label: t('tabs.blank') },
              { value: 'welcome', label: t('tabs.welcome') },
            ],
          },
        ],
      },
